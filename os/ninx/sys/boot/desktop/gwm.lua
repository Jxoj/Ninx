...
local function show_menu(items)
    ...
    for i=1,#items do
        ...
        if type(items[i]) == "table" then
            label = tostring(items[i].label or "")
        else
    ...
    while true do
        ...
        for i=1,#items do
            local label
            local itemColor = WM_COLORS.menu_text
            if type(items[i]) == "table" then
                label = tostring(items[i].label or "")
                if items[i].color then itemColor = items[i].color end
            else
                label = tostring(items[i])
            end
...
local function show_run_menu()
    ...
    local options = {"Ninx Shell","programs...","run...","shutdown","restart"}
    local process = processes[process_current]
    ...
    if ret[1] then
        ...
        local env = {shell=shell, multishell=multishell_ext}
        if ret[2] == 1 then
            
            process_run(env, "ninx/.sys/shell.lua", {}, "Ninx Shell", process.x, process.y)
        elseif ret[2] == 2 then
            
            local pkgItems = {}
            if fs.exists("ninx/packages") and fs.isDir("ninx/packages") then
                local list = fs.list("ninx/packages")
            ...
            if #pkgItems == 0 then
                pkgItems = {"(no packages found)"}
            end
            local ret2 = {pcall(show_menu,pkgItems)}
            if ret2[1] and ret2[2] > 0 then
                local selItem = ret2[3] 
                if type(selItem) == "table" and selItem.file then
                    local path = fs.combine("ninx/packages", selItem.file)
                    process_run(env, path, {}, selItem.label, process.x, process.y)
                else
...
local env = {shell=shell, multishell=multishell_ext}
process_run(env, "ninx/.sys/shell.lua", {}, "Ninx Shell")
process_set_focus(1)
wm_mainloop()
